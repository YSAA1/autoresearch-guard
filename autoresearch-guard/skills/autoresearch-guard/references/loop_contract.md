...
5. audit 保存的输入 digests 与当前六项输入一致：protocol、hypothesis、blocked actions、claim boundary、ledger 和 AI review；audit 文件 digest 与 state 锚点一致。
6. decision 的 `audit_digest`、`input_digests`、forbidden decision 与 critical gate 仍合法；decision 文件 digest 与 state 锚点一致。
7. 有失败 evidence 时，`lessons/anti_patterns.yaml` 有当前 iteration 的条目。
8. decision 为 `proceed`、`refine` 或 `pivot` 时，`next_goal.md` 已填写。
...
- PreToolUse 只提供早反馈。
- PostToolUse 只提醒记录 evidence，不把 tool success 当成 experiment success。
...
```text
protocol + hypothesis + blocked_actions + claim_boundary + ledger + AI review
                               |
                               v
                         audit_report.yaml
                               |
