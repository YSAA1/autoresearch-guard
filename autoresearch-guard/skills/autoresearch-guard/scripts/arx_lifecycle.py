...
from arx_research import evaluate_research_gates
...
SNAPSHOT_FILES = {
    **AUDIT_INPUT_FILES,
    ...
    "subagent_review": "subagent_review.yaml",
}
...
def set_hooks_enabled(research_root: str | Path, *, enabled: bool) -> dict[str, Any]:
    """Toggle campaign-local hooks without changing phase or owner.

    Allowed in any live phase (including aborted leftovers) so operators can
    disarm residual monorepo/.research campaigns without a full archive.
    """
    root = Path(research_root).resolve()
    with research_lock(root):
        ...
        status = str(loop.get("status") or "")
        if status == "complete":
            raise ArxError("hooks cannot be toggled after the iteration is archived as complete")
        previous = state.get("hooks_enabled") is True
        desired = bool(enabled)
        if previous == desired:
            return state
        state["hooks_enabled"] = desired
        _write_state(root, state)
        record_event(
            root,
            "loop.hooks_toggled",
            details={"hooks_enabled": desired, "previous": previous},
            state=state,
        )
        return state
...
def evaluate_readiness(research_root: str | Path) -> dict[str, Any]:
    from arx_harness import evaluate_outcome, requires_subagent_review, subagent_review_errors
    ...
    with research_lock(root):
        ...
        if research_status.get("status") != "pass":
            for error in listify(research_status.get("errors")):
                reasons.append(f"research_gate: {error}")
            if not research_status.get("errors"):
                reasons.append("research_gate: research products incomplete")

        verified_status = audit.get("verified_claim_status") or {}
        if verified_status.get("status") == "fail":
            reasons.append("verified_claim_status failed; cannot archive with unresolved verified claims")
        ...
        if requires_subagent_review(root, decision=decision_name, audit=audit):
            audit_digest = _file_digest(cur / "audit_report.yaml")
            for error in subagent_review_errors(root, required=True, audit_digest=audit_digest):
                reasons.append(f"subagent_review: {error}")
        ...
        outcome_report = evaluate_outcome(root)
        outcome_reasons = list(outcome_report.get("failures") or [])
        ...
        if state.get("phase") == "draft":
            next_actions.append("填写 research.yaml 与协议，再编译 active_goal")
        elif state.get("phase") == "execution":
            next_actions.append("完成当前 evidence 后运行 arx_audit.py")
        elif state.get("phase") == "review":
            next_actions.append("填写 ai_evidence_review.md，重新 audit，再运行 arx_decide.py")
            next_actions.append("若准备 promote：先 arx_loop.py prepare-review，并由 review subagent 写 subagent_review.yaml")
        if outcome_reasons and not missing and not stale:
            next_actions.append("满足 outcome.yaml / success_criteria 后再结束 Goal（勿把 process_ready 当成 achieved）")

        process_ready = not reasons and not missing and not stale and decision_name in DECISIONS
        outcome_ready = bool(outcome_report.get("ready"))
        goal_ready = process_ready and outcome_ready
        ready = goal_ready
        ...
        if process_ready and not outcome_ready:
            all_reasons.extend(outcome_reasons)
            all_reasons.append("process_ready but outcome_ready=false; Goal is incomplete")
...
def observe_stop(research_root: str | Path, event: dict[str, Any]) -> dict[str, Any]:
    ...
    with research_lock(root):
        ...
        detail = "; ".join((readiness.get("reasons") or [])[:4])
        missing = ", ".join((readiness.get("missing") or [])[:6])
        next_action = readiness["next_actions"][0] if readiness["next_actions"] else "运行 arx_loop.py check --json 并处理未完成项"
        process_ready = readiness.get("process_ready")
        outcome_ready = readiness.get("outcome_ready")
        reason = (
            f"AutoResearch Guard incomplete (process_ready={process_ready}, outcome_ready={outcome_ready}): "
            f"{detail or readiness.get('outcome')}. "
        )
        if missing:
