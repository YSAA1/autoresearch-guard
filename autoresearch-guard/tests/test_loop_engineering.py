...
class LoopEngineeringContractTest(unittest.TestCase):
    ...
    def write_compilable_fixture(
        ...
        *,
        ...
        (cur / "ai_evidence_review.md").write_text(
            ...
            "| --- | --- | --- | --- | --- |\n"
            "| c1 | validation score meets the declared gate | validation | ledger:attempt | supported |\n",
            encoding="utf-8",
        )
    ...
    def test_pre_tool_hook_guards_locked_protocol_and_owner_session(self) -> None:
        manifest = json.loads((HOOKS / "hooks.json").read_text(encoding="utf-8"))
        pre_matchers = " ".join(str(entry.get("matcher", "")) for entry in manifest["hooks"]["PreToolUse"]).lower()
        self.assertIn("bash", pre_matchers)
        self.assertIn("patch", pre_matchers)

        research = self.init_iteration("pre-i1", enable_hooks=True)
        self.write_compilable_fixture(research, "pre-i1", locked=True)
        self.compile_goal(research)
        self.start_loop(research, "owner-session")
        ...
        payloads = [
            {
                ...
                "tool_input": {
                    "command": "printf 'locked: false' > .research/current/protocol.lock.yaml"
                },
            },
            {
                "session_id": "owner-session",
                ...
                "tool_input": {
                    "patch": "*** Begin Patch\n*** Update File: .research/current/protocol.lock.yaml\n@@\n-locked: true\n+locked: false\n*** End Patch"
                },
            },
            {
                "session_id": "owner-session",
                ...
                "tool_input": {
                    "file_path": str(research / "current" / "protocol.lock.yaml"),
                    "old_string": "locked: true",
                    "new_string": "locked: false",
                },
            },
            {
                ...
                "tool_input": {
                    "command": "python train.py --split validation --experiment unowned"
                },
            },
            {
                "session_id": "owner-session",
                ...
                "tool_name": "Bash",
                "tool_use_id": "tool-rm",
                "tool_input": {"command": "rm .research/current/protocol.lock.yaml"},
            },
        ]
        for payload in payloads:
            with self.subTest(tool_use_id=payload["tool_use_id"]):
                result, output = self.run_hook("pre_tool_command_gate.py", payload)
                self.assert_ok(result)
                self.assertEqual(self.permission_decision(output), "deny", output)

        owner_result, owner_output = self.run_hook(
            "pre_tool_command_gate.py",
            {
                ...
                "tool_input": {
                    "command": "python train.py --split validation --experiment owned"
                },
            },
        )
        self.assert_ok(owner_result)
        self.assertNotEqual(self.permission_decision(owner_output), "deny", owner_output)

        read_result, read_output = self.run_hook(
            "pre_tool_command_gate.py",
            {
                "session_id": "owner-session",
                "turn_id": "turn-read",
                ...
                "tool_input": {"command": "sed -n '1,80p' .research/current/protocol.lock.yaml"},
            },
        )
        self.assert_ok(read_result)
        self.assertNotEqual(self.permission_decision(read_output), "deny", read_output)
    ...
    def test_session_start_and_hooks_run_from_a_unicode_install_copy(self) -> None:
        ...
        pre_result, pre_payload = installed_hook(
            "pre_tool_command_gate.py",
            {
                "session_id": "copy-owner",
                "turn_id": "pre-turn",
                ...
                "hook_event_name": "PreToolUse",
                "tool_name": "Bash",
                "tool_use_id": "copy-pre",
                "tool_input": {"command": "python eval.py --split validation"},
            },
        )
        self.assert_ok(pre_result)
        self.assertIsNone(pre_payload)

        post_result, post_payload = installed_hook(
            "post_tool_capture.py",
            {
                ...
                "tool_name": "Bash",
                "tool_use_id": "copy-post",
                "tool_input": {"command": "python eval.py --split validation"},
                "tool_response": {"exit_code": 0},
            },
        )
        self.assert_ok(post_result)
        self.assertIn("additionalContext", post_payload["hookSpecificOutput"])
